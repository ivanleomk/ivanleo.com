...
categories:
  - Machine Learning
description: A quick guide to a competitior to the Transformer
---
...
<Callout>
For a better and more thorough explanation, do check out [GPT in 60 Lines of numpy](https://jaykmody.com/blog/gpt-from-scratch/) by Jay K mody. Some of the content here is paraphrased and I suggest going to the source for the best understanding.
</Callout>
...
How do they do this? Well, fundamentally, there are three things that determines the computational cost of a transformer

1. The Context Length: This is the maximum number of tokens that it can fit inside it's limited memory - you can think of it like working memory 
2. The Vocabulary size : Our transformer doesn't take in text, instead it takes in an array of integers, each corresponding to some character or substring
3. The number of parameters : The larger the number of parameters, the more computational memory we need to fit everything inside.

You can think of a transformer as having the following function signature where 
...
<CodeTitle lang = "py" title = "Transformer Signature" />
```py
...
<Callout>
Some parts of this explanation are from Code Emporium's Excellent explanation on Transformers which you can view [here](https://www.youtube.com/watch?v=TQQlZhbC5ps)
</Callout>
...
<KommyImage src = "/images/attention_illustration.png" alt = "Attention" />
...
<Callout>
In some cases, (1) might not hold (Eg. in Translation) where grammar structures differ between languages but for a GPT, this is normally the case.
</Callout>
...
<KommyImage src = "/images/Scaling_RWKV.png" />
...
<KommyImage src = "/images/Time_Mixing_Block.png" height = {300} width={300}  alt  = "Time Mixing Block Architecture" />
...
<KommyImage src = "/images/time_mixing_eqn.png" alt = "Time Mixing Equations" />
...
<Callout>
What's interesting about this is that information on all previous tokens seen is stored in a 1024 dimensional array. Compare this to transformers that generate a `n_seq x n_embd` array due to the attention mechanism. Is attention truly even needed?
</Callout>
...
<KommyImage src = "/images/Channel_Mixing.png" />
...
<KommyImage src = "/images/forget.png" />